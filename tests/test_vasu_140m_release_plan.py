...
def test_source_review_deduplication_and_contamination_evidence() -> None:
    report = validate_release_plan(_plan(), ROOT)
...
def test_plan_is_non_authorizing_and_outputs_are_absent() -> None:
    plan = _plan()
    report = validate_release_plan(plan, ROOT)
...
def test_frozen_report_rejects_self_consistent_rehashed_mutation() -> None:
    report = deepcopy(validate_release_plan(_plan(), ROOT))
